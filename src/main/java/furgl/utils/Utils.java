...
public class Utils {
	...
	public static void openShulkerBox(ServerPlayerEntity player, ItemStack stack) {
		...
		player.connection.send(new SPlaySoundPacket(SoundEvents.SHULKER_BOX_OPEN.getRegistryName(), SoundCategory.BLOCKS, player.position(), 0.5f, 1.0f));
	}
	...
	public static void updateTooltip(ItemStack stack, boolean open) {
		if (stack != null && !stack.isEmpty() && stack.getItem() instanceof BlockItem &&
			...
			ListNBT list = stack.getOrCreateTagElement("display").getList("Lore", NBT.TAG_STRING);
