	...
	public static void drawGui(GuiContainerEvent.DrawBackground event) {
		try {
			if (event.getGuiContainer() instanceof ShulkerBoxScreen && selectedShulkerBoxSlot != -1) {
				ItemStack stack = Minecraft.getInstance().player.inventory.getStackInSlot(selectedShulkerBoxSlot);
				Slot slot = null;
				for (Slot s : Minecraft.getInstance().player.container.inventorySlots)
				...
				if (slot != null) {
					...
					RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
					Screen.fill(slot.xPos, slot.yPos, slot.xPos + 16, slot.yPos + 16, new Color(47, 98, 255).getRGB());
