...
public class CPacketOpenShulkerBox {
	...
	public static class Handler {
		...
		public static void handle(CPacketOpenShulkerBox packet, Supplier<NetworkEvent.Context> ctx) {
			...
			if (packet.slot == -1) {
				player.connection.sendPacket(new SPlaySoundPacket(SoundEvents.BLOCK_SHULKER_BOX_CLOSE.getRegistryName(), SoundCategory.BLOCKS, player.getPositionVec(), 0.5f, 1.0f));
